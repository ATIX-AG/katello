...
module Actions::Katello::Repository
  ...
  class SyncDebErrataTest < TestBase
    ...
    let(:repository) do
      katello_repositories(:debian_10_amd64)
    end
    ...
    context 'stores erratum (finalize phase)' do
      ...
      it 'ignores changes, if not modified' do
        ...
        before_errata_ids = repository.errata.pluck(:errata_id)
        # if JSON.parse() is called, something is off
        ...
        assert_equal before_errata_ids, repository.errata.pluck(:errata_id)
      end
      it 'removes old errata if mirror is set' do
        ...
        existing_erratum = katello_errata(:deb_1)
        assert_include repository.errata, existing_erratum
        action.finalize
        repository.errata.reload # force a reload
        assert_equal ['DEBERR-1-1'], repository.errata.pluck(:errata_id)
      end
      it 'keeps old errata if mirror is not set' do
        ...
        existing_errata = repository.errata.pluck(:errata_id)
        refute_empty existing_errata
